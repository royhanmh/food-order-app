...
const Modal = (props) => {
    ...
    const order = (ev) => {
        ...
        axios.post("http://localhost:3001/order/insert", data).then(res => { document.querySelector(".modal").classList.add("hidden") }).catch(err => console.log(err))
    }
        ...
        <div className={`modal ${(!props.state) ? 'hidden' : 'flex justify-center items-center'}`}>
            <div className="modal-content">
                ...
                <div className="modal-body">
                    <h3 className="font-mono mb-2 text-[20px] text-bold">Nama Makanan : {props.food.foodName}</h3>
                    ...
                    <select name="kecamatan" onClick={() => getKecamatan()} onChange={(e) =>  { setIdKec(e.target.value);setNameKec(e.target.options[e.target.selectedIndex].text) } } class="mb-7 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-4">
                        ...
                        <div>
                            <label for="amount" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Jumlah makanan</label>
                            <input onChange={(e) => setAmount(e.target.value)} type="number" name="jumlah" id="amount" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"/>
                        </div>
                    </div>
                    <label for="total" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Total harga</label>
                    <input type="text" id="total" name="total" class="mb-6 bg-gray-100 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 cursor-not-allowed dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-600 dark:text-gray-400 dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder={`Rp. ${props.food.foodPrice * amount}`} value={props.food.foodPrice * amount}/>
                </div>
                <div>
